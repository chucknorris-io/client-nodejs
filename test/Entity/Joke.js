...
describe(util.format('%s/Entity/Joke', pkg.name), function () {
    ...
    describe('#Constructor', function() {
        ...
        it('Sets instance variables', function() {
            const joke = new Joke('id', 'value', 'sourceUrl', 'iconUrl');
            assert.equal(joke.getId(), 'id');
            assert.equal(joke.getIconUrl(), 'iconUrl');
            assert.equal(joke.getSourceUrl(), 'sourceUrl');
            assert.equal(joke.getValue(), 'value');
        });
    });

    describe('#getId', function() {
        it('Should return the joke`s id', function() {
            const joke = new Joke('qyiEeNq7QXWc2ODugw1gFw');
    ...
    describe('#getIconUrl', function() {
        it('Should return the joke`s icon url', function() {
            const joke = new Joke(null, null, null, 'https://assets.chucknorris.host/img/avatar/chuck-norris.png');
    ...
    describe('#getSourceUrl', function() {
        it('Should return the joke`s source url', function() {
            const joke = new Joke(null, null, 'http://api.chucknorris.io/jokes/qyiEeNq7QXWc2ODugw1gFw');
    ...
    describe('#getValue', function() {
        it('Should return the joke`s value', function() {
            const joke = new Joke(null, 'Chuck Norris can watch videos on Instagram!', null, null);
